...
export default function IssueDetailPage({ params }: { params: Promise<{ issueId: string }> }) {
  ...
  // Keyboard shortcuts for issue detail navigation & copy
  useHotkey("J", () => {
    if (currentIndex < issues.length - 1) router.push(`/issues/${issues[currentIndex + 1].id}`);
  });
  useHotkey("K", () => {
    if (currentIndex > 0) router.push(`/issues/${issues[currentIndex - 1].id}`);
  });
  useHotkey("Mod+.", () => {
    if (issue) navigator.clipboard.writeText(issue.id);
  });
