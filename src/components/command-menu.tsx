...
// ─── Context to let any component open the command menu ──────────────────────
type CommandMenuContextType = { open: () => void };
const CommandMenuContext = React.createContext<CommandMenuContextType>({ open: () => {} });
export const useCommandMenu = () => React.useContext(CommandMenuContext);

// ─── Provider (wraps tree so siblings can call useCommandMenu) ────────────────
export function CommandMenuProvider({ children }: { children: React.ReactNode }) {
  const [isOpen, setIsOpen] = React.useState(false);

  useHotkey("Mod+K", () => setIsOpen((v) => !v));
...
// ─── Dialog (rendered by the provider) ────────────────────────────────────────
