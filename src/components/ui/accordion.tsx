...
function AccordionIcon<T extends React.ElementType>({
  ...
  return <Component className={cn("text-text-sub-600 size-5", className)} {...rest} />;
}
...
>(({ children, className, ...rest }, forwardedRef) => {
  return (
    <AccordionPrimitive.Content
      ...
      {...rest}
    >
      <div className={cn("text-paragraph-sm text-text-sub-600 pt-1.5", className)}>{children}</div>
    </AccordionPrimitive.Content>
  );
});
